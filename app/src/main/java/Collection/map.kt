...
fun main(){
    ...
    /**Use map function**/
    println("USE map() FUNCTION : ")
    peopleHeight.map{print("\n${it.key} is ${it.value}\n")}
    println()
    treesNumbers.map { println("${it.key} is ${it.value}")}
    println()
    enterpriseEmployees.map { println("${it.value} is for ${it.key}") }
    println()

    /**Another way to do it**/
    print(peopleHeight.map { "${it.key} is ${it.value}" }.joinToString(", "))
    println()
